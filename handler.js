    ...
    case 'remove':
      if (chat.welcome) {
        ...
        for (const user of participants) {
          ...
          let ppgp = 'https://i.ibb.co/3hJTvhv/2-20230925-174831-0001.png'  
          try {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Shizu 👑;;;\nFN:Shizu 👑\nORG:Shizu 👑\nTITLE:\nitem1.TEL;waid=5492613619545:+54 9 261 361 95 45\nitem1.X-ABLabel:Shizu 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:Shizu 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: 'Shizu 👑', contacts: [{vcard}]}}, {quoted: callmsg});
