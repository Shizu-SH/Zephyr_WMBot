...
const connectionOptions = {
  ...
  browser: ['ZephyrBot', 'Safari', '1.0.0'],
  version,
...
let prekey = []
let directorio = readdirSync("./ZephyrSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./ZephyrSession/${files}`)
})
} 
...
const directories = ['./ZephyrSession/', './jadibts/']
...
setInterval(async () => {
  ...
  const bio = `🤖 ᴛɪᴇᴍᴘᴏ ᴀᴄᴛɪᴠᴏ: ${uptime} ┃ Zephyr Bot ┃ 🔗 ᴄᴜᴇɴᴛᴀs ᴏғᴄ: http://gamshop.website`;
